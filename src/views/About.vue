<template>
  <div class="about section margin-wrap" id="about" name="about">
    <b-container fluid class="wrap-mobile">
      <b-row>
        <b-col>
          <b-row class="hidden">
            <b-col>
              <h2>About</h2>
            </b-col>
          </b-row>
        </b-col>
      </b-row>
      <b-row class="aboutRow">
        <b-col class="desktop content-left" lg="6" align="left">
          <div class="description">
            <div class="hidden">Hi! My name is Andrew.</div>
            <br />
            <div class="hidden">
              I am an incoming software engineer at Hypotenuse Labs. I graduated
              from the University of Waterloo in 2022 with a degree in
              Mechatronics Engineering.
            </div>
            <br />
            <div class="hidden">
              I love working with blockchain technology. I also enjoy creating
              music, pursuing&nbsp;
              <a
                class="link"
                href="https://photography.andrew-drury.com/"
                target="_blank"
                >photography</a
              >, and going to the gym.
            </div>
            <br />
            <!-- <div class="hidden">
              I am currently seeking full-time opportunities in a software
              engineering role as I will graduate in May of 2022.
            </div>
            <br /> -->
            <div class="hidden">
              Let's talk! Get in touch with me at&nbsp;
              <a class="link" href="mailto:hello@andrew-drury.com"
                >hello@andrew-drury.com</a
              >.
            </div>
          </div>
        </b-col>

        <b-col class="pic" lg="6" align="right">
          <a
            class="link"
            href="https://photography.andrew-drury.com/"
            target="_blank"
          >
            <img src="@/assets/AndrewHeadshot.jpg" />
          </a>
        </b-col>
        <b-col class="mobile" align="left">
          <div class="description">
            <div class="hidden">Hi! My name is Andrew Drury.</div>
            <br />
            <div class="hidden">
              I am an incoming software engineer at Hypotenuse Labs. I graduated
              from the University of Waterloo in 2022 with a degree in
              Mechatronics Engineering.
            </div>
            <br />
            <div class="hidden">
              I love working with blockchain technology. I also enjoy creating
              music, pursuing&nbsp;
              <a
                class="link"
                href="https://photography.andrew-drury.com/"
                target="_blank"
                >photography</a
              >, and going to the gym.
            </div>
            <br />
            <!-- <div class="hidden">
              I am currently seeking full-time opportunities in a software
              engineering role as I will graduate in May of 2022.
            </div>
            <br /> -->
            <div class="hidden">
              Let's talk! Get in touch with me at&nbsp;
              <a class="link" href="mailto:hello@andrew-drury.com"
                >hello@andrew-drury.com</a
              >.
            </div>
          </div>
        </b-col>
      </b-row>
    </b-container>
  </div>
</template>

<style lang="scss">
@import "../assets/variables.scss";
</style>
